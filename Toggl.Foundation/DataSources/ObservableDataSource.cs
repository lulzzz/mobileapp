...
namespace Toggl.Foundation.DataSources
{
        ...
        where TThreadsafe : IThreadsafeModel, IIdentifiable, TDatabase
    {
        ...
        public override IObservable<TThreadsafe> Update(long id, TThreadsafe entity)
            => base.Update(id, entity)
                .Do(updatedEntity => UpdatedSubject.OnNext(new EntityUpdate<TThreadsafe>(id, updatedEntity)));
        ...
        public override IObservable<IEnumerable<IConflictResolutionResult<TThreadsafe>>> BatchUpdate(IEnumerable<TThreadsafe> entities)
            => base.BatchUpdate(entities)
                .Do(updatedEntities => updatedEntities
                    .Where(result => !(result is IgnoreResult<TThreadsafe>))
                    .ForEach(handleBatchUpdateResult));

        private void handleBatchUpdateResult(IConflictResolutionResult<TThreadsafe> result)
        {
            switch (result)
            {
