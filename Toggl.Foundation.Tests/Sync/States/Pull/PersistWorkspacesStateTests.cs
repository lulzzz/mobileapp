    ...
    public sealed class PersistWorkspacesStateTests : PersistStateTests
    {
            ...
            : TheStartMethod<PersistWorkspacesState, IWorkspace, IDatabaseWorkspace>
        {
            ...
            protected override List<IWorkspace> CreateListWithOneItem(DateTimeOffset? at = null)
                => new List<IWorkspace> { new Workspace { At = at ?? Now, Name = Guid.NewGuid().ToString() } };
            ...
            protected override FetchObservables CreateObservables(List<IWorkspace> workspaces = null)
            ...
            protected override List<IWorkspace> CreateComplexListWhereTheLastUpdateEntityIsDeleted(DateTimeOffset? at)
                => new List<IWorkspace>
                {
                    new Workspace { At = at?.AddDays(-1) ?? Now, Name = Guid.NewGuid().ToString() },
                    new Workspace { At = at?.AddDays(-3) ?? Now, Name = Guid.NewGuid().ToString() },
                    new Workspace { At = at ?? Now, ServerDeletedAt = at, Name = Guid.NewGuid().ToString() },
                    new Workspace { At = at?.AddDays(-2) ?? Now, Name = Guid.NewGuid().ToString() }
                };
